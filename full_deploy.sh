...
# # Step 4: Run all Ansible playbooks in one step
...
# Step 6: Get ALB Hostnames for Ingresses
...
# Step 7: Remind to add hosts entries with correct IPs
...
# Step 8: Provide clickable links for services
...
curl -L -s -o /dev/null -w "Grafana HTTP status: %{http_code}\n" -H "Host: grafana.contapics.local" http://$GRAFANA_ALB_IP
echo ""
